-- CustomUnitFrames.lua
-- Implements ActionHud's custom unit frames with Midnight compatibility
...
local CustomUnitFrames = ActionHud:NewModule("CustomUnitFrames", "AceEvent-3.0")
...
function CustomUnitFrames:CreateFrames()
	local units = {
		player = "player",
		target = "target",
		focus = "focus",
	...
	for frameId, unit in pairs(units) do
		local f = CreateFrame("Frame", "ActionHudCustomUnitFrame_" .. frameId, UIParent, "BackdropTemplate")
		f.unit = unit
		f.unitId = frameId
...
function CustomUnitFrames:UpdateFrameEvent(event, unit)
	for _, f in pairs(self.frames) do
		if f.unit == unit then
			self:UpdateFrameValues(f)
		end
...
function CustomUnitFrames:UpdateFrameValues(f)
	local unit = f.unit
	...
	local db = self.db.profile.customUfConfig[f.unitId]

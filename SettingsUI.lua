...
function ActionHud:SetupOptions()
	...
	local function CopyFrameConfig(fromId, toId)
		...
		local source = ActionHud.db.profile.customUfConfig[fromId]
		if source then
			-- Deep copy the configuration
			ActionHud.db.profile.customUfConfig[toId] = ns.Utils.DeepCopy(source)
			local mod = ActionHud:GetModule("CustomUnitFrames", true)
			if mod and mod:IsEnabled() then
				mod:UpdateLayout()
			end
			LibStub("AceConfigRegistry-3.0"):NotifyChange("ActionHud_CustomUnitFrames")
		end
	end
		...
		return {
			...
			args = {
				enabled = {
					...
					order = 1,
					get = function() 
						local db = self.db.profile.customUfConfig[frameId]
				...
				position = {
					...
					order = 2,
					values = {
						TopLeft = L["Top Left"], TopCenter = L["Top Center"], TopRight = L["Top Right"],
						Left = L["Left"], Center = L["Center"], Right = L["Right"],
						BottomLeft = L["Bottom Left"], BottomCenter = L["Bottom Center"], BottomRight = L["Bottom Right"],
					},
					get = function() 
						local db = self.db.profile.customUfConfig[frameId]
				...
				enableGroup = {
					...
					args = {
						enabled = {
							...
							type = "toggle",
							order = 1,
							get = function() return self.db.profile.customUfConfig[frameId].enabled end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].enabled = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						copyFrom = {
							...
							values = function()
								local vals = { [""] = L["-- Select --"] }
								if frameId ~= "player" then vals.player = L["Player Frame"] end
								if frameId ~= "target" then vals.target = L["Target Frame"] end
								if frameId ~= "focus" then vals.focus = L["Focus Frame"] end
								return vals
							end,
							get = function() return "" end,
							set = function(_, val) if val ~= "" then CopyFrameConfig(val, frameId) end end,
						},
					},
				},
				dimensions = {
					...
					args = {
						width = {
							...
							order = 1,
							get = function() return self.db.profile.customUfConfig[frameId].width end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].width = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						height = {
							name = L["Height"],
							type = "range",
							...
							order = 2,
							get = function() return self.db.profile.customUfConfig[frameId].height end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].height = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						xOffset = {
							name = L["X Offset"],
							type = "range",
							...
							order = 3,
							get = function() return self.db.profile.customUfConfig[frameId].xOffset end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].xOffset = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						yOffset = {
							name = L["Y Offset"],
							type = "range",
							...
							order = 4,
							get = function() return self.db.profile.customUfConfig[frameId].yOffset end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].yOffset = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
					},
				},
				visuals = {
					...
					args = {
						bgColor = {
							...
							order = 1,
							get = function()
								local c = self.db.profile.customUfConfig[frameId].bgColor
								return c.r, c.g, c.b
							end,
							set = function(_, r, g, b)
								self.db.profile.customUfConfig[frameId].bgColor = { r = r, g = g, b = b }
								ActionHud:GetModule("CustomUnitFrames"):UpdateLayout()
							end,
						},
						bgOpacity = {
							...
							isPercent = true,
							order = 2,
							get = function() return self.db.profile.customUfConfig[frameId].bgOpacity end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].bgOpacity = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						borderColor = {
							name = L["Border Color"],
							type = "color",
							order = 3,
							get = function()
								local c = self.db.profile.customUfConfig[frameId].borderColor
								return c.r, c.g, c.b
							end,
							set = function(_, r, g, b)
								self.db.profile.customUfConfig[frameId].borderColor = { r = r, g = g, b = b }
								ActionHud:GetModule("CustomUnitFrames"):UpdateLayout()
							end,
						},
						borderOpacity = {
							...
							isPercent = true,
							order = 4,
							get = function() return self.db.profile.customUfConfig[frameId].borderOpacity end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].borderOpacity = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						borderSize = {
							name = L["Border Size"],
							type = "range",
							min = 0, max = 10, step = 1,
							order = 5,
							get = function() return self.db.profile.customUfConfig[frameId].borderSize end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].borderSize = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
					},
				},
				bars = {
					...
					args = {
						powerBarEnabled = {
							...
							type = "toggle",
							order = 1,
							get = function() return self.db.profile.customUfConfig[frameId].powerBarEnabled end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].powerBarEnabled = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						powerBarHeight = {
							...
							min = 0, max = 50, step = 1,
							order = 2,
							get = function() return self.db.profile.customUfConfig[frameId].powerBarHeight end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].powerBarHeight = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						classBarEnabled = {
							...
							get = function() return self.db.profile.customUfConfig[frameId].classBarEnabled end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].classBarEnabled = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						classBarHeight = {
							name = L["Class Bar Height"],
							type = "range",
							...
							order = 4,
							hidden = (frameId ~= "player"),
							get = function() return self.db.profile.customUfConfig[frameId].classBarHeight end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].classBarHeight = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
					},
				},
				text = {
					...
					args = {
						paddingH = {
							...
							min = 0, max = 20, step = 1,
							order = 1,
							get = function() return self.db.profile.customUfConfig[frameId].textPaddingH end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].textPaddingH = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
						paddingV = {
							...
							min = 0, max = 20, step = 1,
							order = 2,
							get = function() return self.db.profile.customUfConfig[frameId].textPaddingV end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].textPaddingV = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
				...
				icons = {
					...
					args = {
						margin = {
							...
							min = 0, max = 20, step = 1,
							order = 1,
							get = function() return self.db.profile.customUfConfig[frameId].iconMargin end,
							set = function(_, val) self.db.profile.customUfConfig[frameId].iconMargin = val; ActionHud:GetModule("CustomUnitFrames"):UpdateLayout() end,
						},
	...
	-- Only show Debugging panel in dev mode (MechanicLib)
	if IS_DEV_MODE or (MechanicLib and MechanicLib:IsEnabled()) then

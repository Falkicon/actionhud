...
lib.MediaTable.background["Blizzard Parchment 2"] =
...
-- FONT
if not lib.MediaTable.font then
	lib.MediaTable.font = {}
end
...
function lib:Register(mediatype, key, data, langmask)
	...
	if
		mediatype == lib.MediaType.FONT
		and ((langmask and band(langmask, LOCALE_MASK) == 0) or not (langmask or locale_is_western))
	then
	...
	if mediatype == lib.MediaType.SOUND and type(data) == "string" then
		...
		if
			not path:find(".ogg", nil, true)
			and not path:find(".mp3", nil, true)
			and not path:find(".wav", nil, true)
		then
...
function lib:Fetch(mediatype, key, noDefault)
	...
	local result = mtt
			and ((overridekey and mtt[overridekey] or mtt[key]) or (not noDefault and defaultMedia[mediatype] and mtt[defaultMedia[mediatype]]))
		or nil

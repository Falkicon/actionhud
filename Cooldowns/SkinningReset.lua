...
function SkinningReset.RestoreFrame(itemFrame)
	...
	if not itemFrame._ahOriginal then
		return
	end
...
SuppressEffectFrame = function(itemFrame, effectFrame, name, suppressChildren)
	if not effectFrame or effectFrame._ahHooked then
		return
	end
	...
	-- Hook SetShown (wrap shown check in pcall for Midnight secret values)
	hooksecurefunc(effectFrame, "SetShown", function(self, shown)
		-- Use pcall for boolean test in case shown is a secret value
		local ok, isShown = pcall(function()
			return shown == true
		end)
		if ok and isShown and itemFrame._ahReset then
			self:Hide()
			self:SetAlpha(0)
			self:ClearAllPoints()
			self:SetPoint("CENTER", UIParent, "CENTER", 10000, 10000)

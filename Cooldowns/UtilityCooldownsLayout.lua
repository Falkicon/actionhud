...
function UtilityCooldownsLayout:InstallStylingHooks(blizzFrame)
	if not blizzFrame or self._stylingHooked then
		return
	end
	if not Reset then
		return
...
function UtilityCooldownsLayout:ApplyStylingToExistingIcons(blizzFrame)
	if not blizzFrame or not Reset then
		return
	end
...
function UtilityCooldownsLayout:UpdateLayout()
	...
	if inStack and LM then
		-- Use own content width, not HUD max
		...
		local p = self.db.profile
		local align = p.utilityCooldownsAlignment or "CENTER"
		if align == "LEFT" then
		...
		-- Independent mode: use Blizzard frame's actual dimensions
		local blizzFrame = self:GetBlizzardFrame()
		local actualWidth = 40
		local actualHeight = 40

		if blizzFrame then
			local bw, bh = blizzFrame:GetWidth(), blizzFrame:GetHeight()
			if bw and bw > 0 then
				actualWidth = bw
			end
			if bh and bh > 0 then
		...
		-- Minimum of icon size
		local iconSize = p.utilityCooldownsIconSize or 36
		actualWidth = math.max(actualWidth, iconSize)
		actualHeight = math.max(actualHeight, iconSize)
		...
		if LM then
			LM:SetModuleHeight("utilityCooldowns", 0)
		end
	end

	-- Re-position Blizzard frame (anchor TOPLEFT since Blizzard positions icons at TOPLEFT)
	local blizzFrame = self:GetBlizzardFrame()
	if blizzFrame then
		blizzFrame:ClearAllPoints()
		blizzFrame:SetPoint("TOPLEFT", container, "TOPLEFT")
	end
...
-- KNOWN ISSUE: Utility Cooldowns Width Calculation (2026-01-18)
-- ============================================================
-- Problem: Container width doesn't match the actual visible icon content.
-- The container may be sized for more icons than visible, causing alignment
-- issues when the module is CENTER-aligned in the HUD.
--
-- Potential future solutions:
-- - Build our own cooldown tracker using pre-seeded cache pattern
--   (see Mechanic/docs/addon-dev-guide/13-midnight-secret-values.doc.md)
-- - Hook into Blizzard's layout refresh to capture actual icon positions
-- ============================================================
function UtilityCooldownsLayout:GetLayoutWidth()
	...
	-- Count visible icons and measure actual icon size from first visible icon
	local blizzFrame = self:GetBlizzardFrame()
	local visibleCount = 0
	local actualIconSize = 36 -- fallback
	if blizzFrame then
		for i = 1, blizzFrame:GetNumChildren() do
			local child = select(i, blizzFrame:GetChildren())
			if child and child:IsShown() and child.Icon then
				visibleCount = visibleCount + 1
				-- Get actual icon size from the frame (Blizzard controls this via Edit Mode)
	...
	-- Calculate width from icon count × actual size
	if visibleCount > 0 then
		local spacing = 2
		return (actualIconSize * visibleCount) + (spacing * math.max(visibleCount - 1, 0))
	end

	-- Fallback to minimum width if no icons
	return actualIconSize
end
